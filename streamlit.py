...
if use_camera:
    ...
    if captured_frame is not None:
        ...
        if len(detected_classes) > 0:
            ...
            detected_title = normalized_landmark_info[normalized_class]["English_title"]
            ...
            if normalized_class in normalized_landmark_info:
                ...
                st.write(f"{info['English_description']}")
                st.write(f"{info['Arabic_title']}")
                st.write(f"{info['Arabic_description']}")
                st.write("**Resources**:")
